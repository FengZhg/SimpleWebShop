...
namespace backEnd
{
    public static class Captcha
    {
        ...
        public static Queue<CaptchaResult> CaptchaResults = new Queue<CaptchaResult>();

        public static int CaptchaSemaphoreSize = 100;

        public static Semaphore CaptchaSemaphore = new Semaphore(0, CaptchaSemaphoreSize);

        public static Semaphore AntiCaptchaSemaphore = new Semaphore(CaptchaSemaphoreSize, CaptchaSemaphoreSize);
        public static void StartPushCaptcha()
        {
            while (true)
            {
                AntiCaptchaSemaphore.WaitOne();
                Task task = Task.Run(() =>
                {
                    GenerateCaptcha();
                });
                task.Wait();
                CaptchaSemaphore.Release();
                Console.WriteLine(CaptchaResults.Count());
                Thread.Sleep(10);
            }
        }
        public static void GenerateCaptcha()
        {
            ...
            CaptchaResults.Enqueue(result);
        }
        public static CaptchaResult GetCaptchaResult()
        {
            CaptchaSemaphore.WaitOne();
            var captchaTmp = CaptchaResults.Dequeue();
            AntiCaptchaSemaphore.Release();
            return captchaTmp;
        }
    }
