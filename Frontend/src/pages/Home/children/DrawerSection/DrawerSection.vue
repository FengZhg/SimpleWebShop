        ...
        <el-drawer title="购物车"
            ...
            <div class="meg_show">
                ...
                <div class="meg_content"
                     v-if="userInfo.id">
                    <DrawerItem v-for="(goods) in cartgoods"
                                :key="goods.goods_id"
                                :goods="goods"
                                @computeShopCar="computeShopCar" />
                </div>
                <div class="meg_noLogin"
                     v-else>请先登录~</div>
...
export default {
    ...
    methods: {
        ...
        // 结算该商品
        calCart () {
            this.$confirm('您确定结算所选商品吗？', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'info'
            }).then(() => {
                let user_id = this.userInfo.id;
                let re = this.$store.dispatch('delAllGoods', { user_id });
                // 请求商品数据
